...
hbb_name = 'horizon' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/horizon.exe')
    os.chdir('../..')
    if os.path.exists('./horizon_portable.exe'):
        os.replace('./target/release/horizon-portable-packer.exe',
                   './horizon_portable.exe')
    else:
        os.rename('./target/release/horizon-portable-packer.exe',
                  './horizon_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/horizon_portable.exe')
    os.rename('./horizon_portable.exe', f'./horizon-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/horizon-{version}-install.exe')
